...
async def create_combatants(combatants: list[CombatantModel], session: Session = Depends(get_session)):
...
async def get_combatant(name: str, session: Session = Depends(get_session)):
    ...
    if not combatant:
        raise HTTPException(status_code=404, detail=f"Combatant with name '{name}' not found; cannot be retrieved.")
    return combatant
...
async def delete_all_combatants(session: Session = Depends(get_session)):
    ...
    return {
        "message": f"All {length} combatants deleted successfully."
    }
...
async def delete_combatant(name: str, session: Session = Depends(get_session)):
    ...
    if not combatant:
        raise HTTPException(status_code=404, detail=f"Combatant with name '{name}' not found; cannot be deleted.")
    session.delete(combatant)
    session.commit()
...
async def replace_all_combatant_info(name: str, updated_combatant: CombatantModel, session: Session = Depends(get_session)):
...
async def update_combatant_dex_score(name: str, dex_score: int, session: Session = Depends(get_session)):
    combatant = session.exec(
        select(CombatantModel).where(CombatantModel.name == name)
    ).first()
    if not combatant:
...
async def update_combatant_other_init_mod(name: str, other_init_mod: int, session: Session = Depends(get_session)):
    combatant = session.exec(
        select(CombatantModel).where(CombatantModel.name == name)
    ).first()
    if not combatant:
...
async def update_combatant_char_class(name: str, char_class: CharacterClass, session: Session = Depends(get_session)):
    combatant = session.exec(
        select(CombatantModel).where(CombatantModel.name == name)
    ).first()
    if not combatant:
